...
internal class Spawn : BaseTemplateCommand
{
	...
	internal static void SpawnTemplate(string template, EFT.Player player, ConsoleCommand command, Func<ItemTemplate, bool> filter)
	{
	...
	private static void SpawnTemplate(ItemTemplate template, EFT.Player player, ConsoleCommand command)
	{
		var poolManager = Singleton<ObjectsFactory>.Instance;

		poolManager
			.LoadBundlesAndCreatePools(ObjectsFactory.PoolsCategory.Raid, ObjectsFactory.AssemblyType.Online, [.. template.AllResources], JobYieldPriority.Immediate)
			.ContinueWith(task =>
			{
				AsyncWorker.RunInMainTread(delegate
				{
					...
					else
					{
						...
						{
							_ = new ItemController(item, item.Id, item.ShortName);
	...
	private static void SetupItem(EFT.ItemFactory itemFactory, Item item)
	{
		...
		if (item is IAmmoContainer container)
