...
internal sealed class InstallCommand : AsyncCommand<InstallCommand.Settings>
{
	...
	public override async Task<int> ExecuteAsync(CommandContext commandContext, Settings settings, CancellationToken ct)
	{
		try
		{
			...
			var result = await BuildRavenXAsync(settings, installation, features, commands);
			...
			if (!CreateDll(installation, "RavenX.dll", dllPath => result.Compilation.Emit(dllPath, manifestResources: result.Resources)))
			...
			if (installation.UsingBepInEx && result.Archive!.Entries.Any(e => e.Name == bepInExPluginProject))
			{
				...
				if (!CreateDll(installation, Path.Combine(installation.BepInExPlugins, "RavenX.Plugin.dll"), dllPath => pluginResult.Compilation.Emit(dllPath)))
	...
	private static async Task<CompilationResult> BuildRavenXAsync(Settings settings, Installation installation, params string[] folders)
	{
	...
	private static string? GetRetryBranch(Installation installation, CompilationContext context)
	{
		...
		return dedicated == context.Branch ? null : dedicated;
	}
		...
		try
		{
			await AnsiConsole
				...
				.StartAsync(status, async statusContext =>
				{
					...
					var buffer = await client.GetByteArrayAsync(new Uri($"https://codeload.github.com/C1yHAX/SPT-RAVEN-X/zip/refs/heads/{branch}"));
